...
For details, see our [paper](https://arxiv.org/abs/2603.07280).
...
```
bazel build --config=opt --build_tag_filters=-cuda //...
```

To build all with CUDA:
```
bazel build --config=opt --define=use_gpu=1 //...
```
...
For the checked-in proof certificates and the commands used to reproduce each one, see [proof_cert/README.md](proof_cert/README.md).
