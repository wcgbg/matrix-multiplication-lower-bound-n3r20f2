...
//
// Inputs are assumed to have no bits set beyond `bitwidth`.
...
int GaussJordanElimination(int bitwidth, VectorOfUnsignedInts *matrix) {
  ...
  while (current_row >= 0 && pivot_col >= 0) {
    ...
    const Row col_mask = Row{1} << pivot_col;
    int pivot_row = -1;
    ...
    for (int r = 0; r < size; ++r) {
      if (r != current_row && (data[r] & col_mask)) {
        data[r] ^= data[current_row];
      }
    }
